...
You can open up the terminal and create a qemu-cli prompt for your virtual machine, here's an example of my CachyOS VM:
...
- `-net user,hostfwd=tcp::2222-:22` means that port 22 is being forwarded from the host to the vm as port 2222, thus if you wanted an ssh connection the vm from some device, you'd do:

```sh
ssh -p 2222 vmusername@vmlocalhost
```

#### Check [post-install tweaks/12. missing things and tweaks](../../post-install%20tweaks/12.%20missing%20things%20and%20tweaks/instructions.md). for ssh
- `-device virtio-vga-gl,hostmem=4G,blob=true,venus=true` this is the venus driver, unfortunately not yet in virt-manager, once it is, this entire section apart from user mode will be superceded.
